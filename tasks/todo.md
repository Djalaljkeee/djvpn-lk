...
# Защита от повторных платежей (фронт djvpn-lk)

## Контекст
Инцидент: 5 дублей charge в ЮKassa из-за ретраев SHM `action=payment` (медленный,
не идемпотентный). `action=payment` идёт напрямую в закрытый SHM, мимо кода ЛК —
по решению пользователя защиту делаем только на фронте: закрываем вектор
повторного открытия платёжных сессий пользователем.

## Задачи
- [x] `hooks/usePaymentGuard.ts` — хук-гард + чистая `decidePaymentAction`
      (`open`/`refocus`/`locked`), именованная вкладка, фиксация `ts`, кулдаун
      с авто-сбросом по таймеру.
- [x] `pages/PaymentsPage.tsx` — `handlePay` через `openPayment`, тосты на
      refocus/locked/blocked, блок неактивных платёжек + статус-строка.
- [x] `test/usePaymentGuard.test.ts` — unit-тест решения гарда (4 кейса).
- [x] `tasks/lessons.md` — урок про область фикса и ловушку «дедуп в прокси».

## Верификация
- [x] `npm run build` (tsc + vite) — без ошибок.
- [x] `npm test` — 26 passed (вкл. 4 новых).

## Решения / трейд-офы
- Гард — смягчение, не устранение корня: ретрай `action=payment` со страницы SHM
  фронтом не перехватывается. Если дубли повторятся — нужен дедуп-шлюз перед SHM CGI.
- Кулдаун 60 c с авто-сбросом через `setTimeout` в `useEffect`, чтобы кнопки
  разблокировались без посторонней смены state.
- `decidePaymentAction` вынесена чистой для теста без DOM/`window`.
