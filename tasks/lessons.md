...
- Дополнительный ущерб: каждая неудача дёргает
  ...
  счётчике ≥ 5 отдаёт `429` на час по ключу `<класс>-<метод>-<ip>`. IP здесь —
  `get_user_ip()`, то есть `X-Real-IP` конкретного пользователя (цепочка
  проксей его доносит, проверено), так что 429 бьёт только по тому, кто
  реально долбился в виджет. Общего лимита «на всех» тут нет — не путать с
  устаревшим диагнозом про потерю XFF.
...
> **СТАТУС: закрыто, цепочка исправна (проверено 05.08.2026).** Оставлено как
> описание механики и как образец того, КАК это проверять. Ниже — актуальное
> состояние, потом исторический разбор.

- **Главное, что было упущено в исходном разборе: SHM читает НЕ
  `X-Forwarded-For`.** `Core::Utils::get_user_ip` (`app/lib/Core/Utils.pm`):
  ```perl
  sub get_user_ip {
      return $ENV{HTTP_X_REAL_IP} || $ENV{REMOTE_ADDR};
  }
  ```
  Именно её дёргают `set_user_fail_attempt` (`Core/Base.pm`), запись
  `settings{ip}` при регистрации (`User::reg`) и сверка IP сессии. Поэтому
  смотреть надо на **`X-Real-IP`**, а `forwarded_for` в access-логе — это
  логирование nginx, а не то, по чему SHM принимает решения. Диагноз «rate-limit
  общий на всех, потому что XFF теряется» из этого и вырос — он неверен.
- Фактическая цепочка (все хопы проставляют `X-Real-IP` корректно):

  | Хоп | Что делает с IP |
  |---|---|
  | Caddy `lk.` | `X-Real-IP`/`XFF = {client_ip}` |
  | nginx `djvpn-frontend-1` | `$proxy_add_x_forwarded_for` — цепочка сохраняется; `X-Real-IP` затирает своим `$remote_addr`, чинится следующим хопом |
  | `djvpn-backend-1` | `ProxyHeadersMiddleware(trusted_hosts="*")` достаёт IP из XFF → `client_ip_ctx`; `shm_proxy.py:73-75` и `shm_client._proxy_headers()` заново ставят ОБА заголовка |
  | hairpin на `admin.djvpn.ru` | источник приватный → Caddy доверяет XFF |
  | nginx `shm-admin-1` | `set_real_ip_from` приватные + `real_ip_recursive on`, дальше `X-Real-IP $remote_addr` в shm-api |

- **Проверять фактом, а не чтением конфига.** Активный тест через hairpin:
  ```bash
  docker exec djvpn-backend-1 python3 -c "httpx.get('https://admin.djvpn.ru/shm/v1/zz-iptest-hairpin',
    headers={'X-Forwarded-For':'203.0.113.77','X-Real-IP':'203.0.113.77'})"
  # → в логе shm-admin-1: 203.0.113.77 - - [...] "GET /shm/v1/zz-iptest-hairpin" 404
  ```
  Пассивная проверка на живом трафике: агрегация ~3000 строк лога даёт 20+
  разных публичных адресов, а не один; запросы одного пользователя от браузера
  и от `python-httpx/0.28.1` (это и есть backend-прокси) логируются под одним и
  тем же публичным IP. `172.20.0.1` встречается только на
  `GET /shm/v1/storage/manage/vpn_mrzb_*?user_id=*` — это серверные вызовы без
  пользовательского контекста, а не «потерянные» IP.
- Известное ограничение (не срочное): снаружи спуфинг IP невозможен —
  публичный remote не входит в `trusted_proxies`, Caddy перезаписывает
  заголовок. Но любой контейнер на этом же хосте может через hairpin подделать
  `X-Real-IP` и обойти rate-limit SHM. Локально эксплуатируемо, удалённо — нет.

### Исторический разбор (симптом и механика)
