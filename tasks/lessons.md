...
- У Happ в App Store было **две** карточки с разными app-id: Apple по
  требованию РКН удаляла клиент из российской витрины, разработчик
  перезаливал его под новым именем и новым id, и «безвитринная» ссылка
  `apps.apple.com/app/id…` тут не выручала — это разные приложения, а не
  одно в разных странах. К 06.09.2026 RU-карточки кончились совсем: обе
  (`id6788279553`, `id6746188973`) отдают 404, а поиск по российской
  витрине выдаёт клоны посторонних издателей. Поэтому в
  `backend/vpn_setup.py` остались `APPSTORE_HAPP_INTL` (`Happ — Proxy
  Utility`, Flyfrog LLC) и `HAPP_MACOS_DMG` — витрина ему не нужна;
  iOS уехал на INCY (`APPSTORE_INCY`). Проверять ссылку перед правкой
  инструкций обязательно: `curl -sI` на карточку и `itunes.apple.com/lookup`
  по нужной стране.
...
## Асинхронный генератор, брошенный на `return`, не выполняет код после `yield`

`async for db in _db(): … return` выглядит как «взять сессию» — и молча теряет
`commit()`, потому что генератор дочитывает сборщик мусора, а не обработчик.
Запрос отвечает 200, ошибок в логах нет, данных в базе тоже. Урок на будущее:
ресурс с завершающим действием (коммит, закрытие, освобождение) берём через
`async with`, а генератор оставляем только там, где его дочитывает фреймворк
(`Depends()` у FastAPI).

И шире: «ручка ответила 200» — не доказательство. Проверять надо чтением
следующим запросом, а не кодом ответа предыдущего.

## Поиск по русскому тексту: подстрока не работает, нужны токены + основа

- Симптом: база знаний ЛК, поиск `text.includes(word)` по всем словам запроса.
  «возврат денег» — 0 результатов (в ответе «деньги»), «не работает на
  мобильном интернете» — 0 («мобильный»), «медленно работает» — 0 (в записи
  «медленно», но нет «работает»). На живых формулировках из переписки промах
  был в 8 случаях из 40.
- Причина двойная: (1) падежи — форма в запросе не совпадает с формой в
  тексте; (2) пользователь пишет «впн», «тг», «ютуб», а в текстах ответов
  стоят латинские VPN, Telegram, YouTube. Самый частый запрос переписки —
  «не работает впн» — не находил вообще ничего.
- Рабочее решение (`frontend/src/data/faq.ts`): токенизация обеих сторон →
  словарь синонимов (впн→vpn, тг→telegram, комп→компьютер) → грубая основа
  `w.length <= 4 ? w : w.slice(0, Math.max(4, w.length - 3))` → совпадение по
  НАЧАЛУ слова, а не по подстроке (иначе «он» находит «телефон»). Условие И
  по всем словам запроса, иначе «не» вытаскивает половину базы.
- Чередования в корне («деньги»/«денег») так не лечатся в принципе — такие
  формы дописывай в `tags` руками. Поле `tags` вообще держи как «как это
  называет пользователь»: «слетело», «апстор», «ошибка кора».
- Замер держи тестом: словарь `запрос → ожидаемый id` на реальных
  формулировках. Любая правка данных или поиска, после которой запрос
  перестаёт находить свой ответ, — регресс, а не «ну почти».

## Шапка кабинета: четвёртый значок ломает вордмарк на 360px

- В `Layout.tsx` в правом блоке уже три кнопки (инструкции, уведомления,
  профиль) + баланс. На 360px вордмарк «DJ VPN / fast secure reliable» и так
  переносится в три строки; четвёртый значок добавляет четвёртую.
- Проверять глазами, а не на глаз: `page.getByRole('banner').screenshot()` на
  360px до и после (значок гасится `addStyleTag` с `display:none`).
  `scrollWidth === clientWidth` тут ничего не доказывает — оверфлоу нет,
  ломается именно перенос текста.
- Новый вход в раздел на мобиле — пункт в выпадающем меню профиля
  (`sm:hidden`), значок в шапке — `hidden sm:flex`. Нижний навбар не трогать:
  пять пунктов по `min-w-[72px]` в 360px не помещаются.

## Playwright + Vite dev: `**/api/**` глушит исходники, а не бэкенд

- `page.route('**/api/**', …)` в dev-режиме перехватывает не только запросы
  к бэкенду, но и модули `/src/api/client.ts` — Vite отдаёт исходники по
  путям, содержащим `/api/`. Браузер получает `application/json` вместо
  модуля, ловит strict MIME check и не монтирует React: белый экран без
  единой ошибки приложения в консоли.
- Всегда указывай origin: `page.route('http://127.0.0.1:PORT/api/**', …)`.
- Заглушка `{}` на все ручки тоже роняет кабинет: `useSupportChat` делает
  `setThread(data.thread)`, `thread` становится `undefined`, `SupportChat`
  падает на `thread.enabled` и уносит всё дерево. Для `/support` отдавай
  `{ thread: { enabled: true, status: 'open', unread: 0, max_upload_mb: 10 },
  messages: [] }`.
