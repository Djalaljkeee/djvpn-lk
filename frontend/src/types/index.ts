...
export interface ReferralStats {
  ...
  // Активные рефералы — те, у кого есть хотя бы один платёж (paid > 0).
  // Именно от их числа считается уровень реферальной программы.
  active_referrals: number
  total_paid: number
  total_income: number
  referrals: Referral[]
  // Дельты за последние 30 дней. Для referrals_30d / active_referrals_30d
  // считаем на фронте из referrals[].created — это надёжно. Для
  // paid_30d/income_30d ждём, что SHM добавит поля в шаблон (по списку
  // рефералов их корректно не посчитать: реферал мог зарегистрироваться
  // давно, а платежи — недавно). null означает «данных нет, не рисуем дельту».
  referrals_30d: number
  // Активные рефералы, зарегистрированные за последние 30 дней (paid > 0
  // И created в окне). Приблизительная дельта: дату самого платежа SHM не
  // отдаёт, поэтому опираемся на дату регистрации.
